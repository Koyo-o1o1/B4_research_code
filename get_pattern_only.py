...
# visualize_psf関数を、以下のコードに置き換えてください

def visualize_psf(h_FZA, i):
    """
    h_FZA (NumPy配列)を、0-255のグレースケール画像ファイルとして直接保存する。
    """
    print(f"φ={i*90}° のPSFパターンを画像ファイルとして保存中...")
    
    # h_FZAの範囲は0.0-1.0なので、255を掛けて8bit整数(0-255)に変換
    h_FZA_uint8 = (h_FZA * 255).astype(np.uint8)
    
    # NumPy配列からPillow画像オブジェクトを作成
    pil_image = Image.fromarray(h_FZA_uint8, mode='L')
    
    # ファイルに保存
    filename = f'img/FZA/only/FZA_pattern_phi_{i*90}_only_FZA________________.png'
    ...
    plt.title(f"PSF Pattern (φ={i*90}°)")
    plt.show()
...
def visualize_coded_images(original_img,coded_images,phase_labels):
    ...
    plt.tight_layout()
    plt.savefig('img/coded_image/coded_images_real_test.png')
    plt.show()
...
def visualize_synthesis_result(g_FS):
    ...
    plt.savefig('img/FS/fringe_scan_synthesis_real_test.png')
    plt.show()
...
def visualize_final_result(original_img, reconstructed_image_raw):
    ...
    plt.tight_layout()
    plt.savefig('img/final_image/final_reconstruction_real_test.png')
    plt.show()
